...
Search the project for `#UpdateLink` — each of the four is tagged, so you do not need line
numbers that go stale.

| Email | File |
|---|---|
| Strike added | `Features/PresidentAdmin/Services/AdminService.cs` |
| Strike removed | `Features/PresidentAdmin/Services/AdminService.cs` |
| Task assigned | `Features/PresidentAdmin/Services/AdminService.cs` |
| Task completed → team leaders | `Features/Tasks/Services/TaskItemService.cs` |
...
**Do not write a plain-text version.** `EmailService.cs` derives one from your
